# Frankie — Mercado Vancouver + primera lista de objetivos

## Tamaño real del mercado (Metro Vancouver)
- Barberías: 85+ puntuadas solo en Vancouver ciudad (Yably) + 21 en North Van — reales hay bastantes más las no listadas
- Peluquerías/salones: cientos (YellowPages Metro Vancouver)
- Dentistas, fisios, clínicas estéticas, tatuadores, restaurantes: miles en total
- Con capturar el 0,5% del mercado local a $99 ya son varios miles/mes recurrentes

## TANDA 1 — Barberías North Vancouver (10 objetivos reales)
Prioridad a las que NO tengan botón "book online" (esas sufren los DMs):

| # | Negocio | Dónde | Nota |
|---|---------|-------|------|
| 1 | Guido's Barbershop | 1725 Lonsdale Ave | Top rated local |
| 2 | Save On Cuts by Mary & Ali | 1538 Lonsdale Ave | Familiar, horario largo |
| 3 | New York Barbershop | North Van (newyorkbarbershop.ca) | Tiene web propia |
| 4 | Gentleman Jacks Barbershop | North Van | Top rated |
| 5 | Lonsdale Men's Hair Styling | Lonsdale | Clásica |
| 6 | Capilano Barbers & Mens Hairstyling | North Van | |
| 7 | The Village Barber | North Van | |
| 8 | Man Up Grooming | North Van | |
| 9 | Lynn Valley Barbershop | 1199 Lynn Valley Rd | |
| 10 | (rellenar con Google Maps: "barber north vancouver", filtrar sin book online) | | |

DESCARTAR: Tommy Gun's (cadena corporativa, no decide el local).

## Checklist por objetivo (5 min cada uno, sin hablar con nadie)
1. Buscar su Instagram → ¿tiene link de Booksy/Fresha/Square en bio?
   - NO tiene → objetivo A (email 1 de OUTREACH.md)
   - SÍ tiene → objetivo B (guardar para v2 con integración Square)
2. Buscar email en su web (sección contact) o usar el DM de Instagram como canal
3. Apuntar un detalle específico (una reseña, algo de su web) para la primera línea del email

## Calendario 14 días
- **Día 1-2:** subir Frankie online (Railway) + Stripe payment link + dominio
- **Día 3:** completar tanda 1 (checklist de los 10) — 1 hora
- **Día 4:** enviar 5 emails/DMs (los objetivos A)
- **Día 5:** enviar 5 más
- **Día 6-7:** descanso de envío. Preparar tanda 2: dentistas (mismo checklist)
- **Día 8:** enviar tanda 2 (5)
- **Día 9:** enviar tanda 2 (5)
- **Día 10:** follow-up de la tanda 1 (el email corto de OUTREACH.md)
- **Día 11-12:** tanda 3: restaurantes (reservas de mesa)
- **Día 14:** repaso de números: aperturas, respuestas, visitas a la demo, trials. El nicho que más muerda se lleva las siguientes 40.

## Métricas para decidir (día 14)
- 0 respuestas de 30 → cambiar el email, no el producto (probar DM de Instagram en vez de email)
- 1-2 interesados → seguir, afinar
- 1 trial activado → validado, meter contenido TikTok encima y escalar
